...
FROM EMP
WHERE JOB= 'PRESIDENT');
--26. How many employees work in Chicago?select count(ename)
from emp
join
dept
on loc='chicago';
--27. Which employees work in Chicago?select ename
from emp
join
dept
on loc='chicago';
--28. List the employees’ names and cities in which they work. Order the list by city.select ename,
loc
from emp e
join dept d
on e.DEPTNO=d.DEPTNO
order by loc;
--29. Find the number or different employees and number of departments.select count(distinct deptno)
from emp;
--30. Determine the average salaries of employees.select top 1 ename,
avg(sal) as average_salary
from emp
group by ename
order by average_salary;
--31. Determine number of departments in the company.select count(*) as DEPTNO
from EMP
--32. List department number, department name, location and total commission paid and total salary of each department.select distinct e.deptno,
dname,
loc,
...
order by DEPTNO;
--33.Display the employee name, job description and the salary multiplied by 12.SELECT ENAME,
JOB,
SAL*12 AS 'ANUAL SALARY'
FROM EMP;
--34.List the name, job and commission of employee who earn no commission.SELECT ENAME,
JOB,
COMM
FROM EMP
WHERE COMM IS NULL ;
--35. List the name, job and commission of employee who earn commission.SELECT ENAME,
JOB,
COMM
FROM EMP
WHERE COMM IS NOT NULL ;
--36. List the employee name, job description and salary where the third character of employee name is R.SELECT ENAME,
JOB,
SAL
FROM EMP
WHERE ENAME LIKE '__R%';
--37. List all the information from emp that are five characters long.SELECT *
FROM EMP
WHERE ENAME LIKE '_';
--38. List all the information from emp that are not five characters long.SELECT *
FROM EMP
WHERE ENAME LIKE '__';
--39. List names that have A and followed by R.SELECT *
FROM EMP
WHERE ENAME LIKE '%AR%';
--40. List all the details of employee from emp table where department number is equal to 30, order the list by their salary.select ename,
sal
from emp
where deptno=30
order by sal;
--41. Select name and salary of employees who earn between 1250 and 1600.select ename,
sal
from emp
where sal between 1250 and 1600;
--42. List the name and salary of employees who earn less than 1250 or more than 1600.select sal,ename,
(case
...
from emp;
--43. Compute the total salary of all employees.select sum(sal) as totalsalofallemp
from emp;
--44. Count the number of employees who have job titles(that is, the number of rows where JOB is not NULL).select count(ename)
from emp
where job is not null;
--45. Compute the minimum and the maximum salary in the company.select top 1 e.ename,
...
-- Emp #, Employee, Salary, Job, Date of Hire, Commission, Department No, MGR No.select
ename as Employee,
sal as Salary,
job as Job,
...
mgr as 'MGR No'
from EMP;
--47. What is the earliest date on which someone was hired, and what is the latest date on which someone was hired.select e.ename as EarliestDate , c.ename as LatestDate
from emp e , emp c
where e.empno = c.mgr and c.HIREDATE < e.HIREDATE;
--48. Count the number of employees working in department 10.select count(e.deptno)
from emp e
where e.deptno>10;--49. Write a query that will give you the names and jobs of all employees in New York with a commission above 10.select ename,sal,comm
from emp e
join
dept d
on loc='NEW YORK'
...
select datename(weekday,getdate()) 'Day Name', format(getdate(), 'MM-dd-yy') as 'Date'select datename(weekday,getdate()) 'Day Name', format(getdate(), 'dd-mm-yy') as 'Date'select datename(weekday,getdate()) 'Day Name', format(getdate(), 'yy-MM-dd') as 'Date'
